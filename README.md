...
在 8×海光 K100-AI 64GB（gfx928，DTK 26.04）上，将 **DeepSeek-V4-Flash**（284B MoE，激活 13B，自行量化 W8A8）部署为 OpenAI 兼容服务的完整实录：三条路线全部跑通，含全部补丁、启动器、测试脚本、25 轮调优记录，以及提交给海光的详细 Bug 报告。
...
DSpark 侧可调项已试遍，**天花板在 verify 前向本身**：
...
放宽接受阈值确实提高了命中率，速度却纹丝不动——**再高的投机命中率也换不来速度**。
50 tok/s 对应单步 ≈88 ms，需要内核层约 1.5× 提升（NSA decode / allreduce / MoE），已超出应用层可为范围。
