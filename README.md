...
在 8×海光 K100-AI 64GB（gfx928，DTK 26.04）上，将 **DeepSeek-V4-Flash**（284B MoE，激活 13B，自行量化 W8A8）部署为 OpenAI 兼容服务的完整实录：三条路线全部跑通，含全部补丁、启动器、测试脚本、26 轮调优记录，以及提交给海光的详细 Bug 报告。
...
### ⚠️ 已知限制二：`--kv-cache-dtype bfloat16` 静默产出乱码

本线路的解码内核 `triton_fp8_attention_fwd` 按 **fp8 布局**读 KV。改成 bf16 KV 后服务照常启动、
...
**在 0811 + DSpark 线上 fp8 KV 是必需项**，与 0728 线（4 月版 + MTP，fp8 KV 慢 14% 且破坏 Think）
结论相反：两条线走不同内核，结论不可互相套用。详见 `docs/调优记录-轮次25.md`。

### 为什么停在 33.8 而不是 50

应用层可调项已试遍，**天花板不在调度而在权重访存带宽**。三组实验从不同方向指向同一结论：
...
这一步的耗时几乎全部是"把 256 个专家的权重从 HBM 读一遍"，与怎么切 tile、猜中多少无关。
50 tok/s 对应单步 ≈88 ms（现为 ≈130 ms），需要的是**减少每步权重读取量**（专家驻留/复用、
MegaMoE 那类融合调度），已超出应用层可为范围。详见 `docs/调优记录-轮次26.md`。
...
9. **醒目的警告不等于真瓶颈**：镜像里 MoE 调优配置目录是空的、日志一直刷 "Performance might be sub-optimal"，看起来是最可疑的性能问题；实测调优后解码 +1%、预填 0%。真正的瓶颈是权重访存带宽（`docs/调优记录-轮次26.md`）。
